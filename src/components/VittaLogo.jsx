export default function VittaLogo({ size = 24, color = 'currentColor', className = '', showText = false, textColor = '#ffffff' }) {
  if (showText) {
    return (
      <div className={`vitta-logo-brand ${className}`} style={{ display: 'inline-flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center' }}>
        ...
        <span style={{ fontSize: `${Math.max(12, size * 0.35)}px`, fontWeight: 800, letterSpacing: '2px', color: textColor, marginTop: '4px', lineHeight: 1.1 }}>
          VITTA
        </span>
        <span style={{ fontSize: `${Math.max(8, size * 0.2)}px`, fontWeight: 700, letterSpacing: '2.5px', color: '#38bdf8', lineHeight: 1 }}>
          NUTRI
        </span>
      </div>
    );
